    ...
    <div class="page-wrapper">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h3 class="text-themecolor">{{trans('lang.parking_details')}}</h3>
            </div>
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a
                            href="{!! route('parking-list') !!}">{{trans('lang.parking_list')}}</a>
                    </li>
                    <li class="breadcrumb-item active">{{ trans('lang.parking_details')}}
                                    ...
                                    <div class="card card-block p-card">
                                        <div class="profile-box">
                                            <div class="profile-card rounded">
                                                ...
                                                <h3 class="font-600 text-white text-center parking-name"></h3>
                                                <div class="font-600 text-white text-center mb-3 parking-ratings"></div>
                                            ...
                                            <div class="pro-content rounded">
                                                <div class="d-flex align-items-center mb-3">
                                                    ...
                                                    <p class="mb-0 eml parking-owner"></p>
                                                </div>
                                                <div class="d-flex align-items-center mb-3">
                                                    <div class="p-icon mr-3">
                                                ...
                                                <div class="table-responsive user-list-table">
                                                    <table class="table mb-0">
                                                        ...
                                                        <tr>
                                                            <td class="py-2 px-0">
                                                                <span
                                                                    class="font-weight-bold w-100">{{trans("lang.parking_for")}}:</span>
                                                            </td>
                                                        ...
                                                        <tr>
                                                            <td class="py-2 px-0">
                                                                <span
                                                                    class="font-weight-bold w-100">{{trans("lang.parking_space")}}:</span>
                                                            </td>
                        ...
                        <div class="col-lg-8">
                            <div class="card card-block card-stretch">
                                <div class="card-header">
                                    <h3 class="card-title">{{trans("lang.parking_slots")}}</h3>
        ...
        <div class="form-group col-12 text-center btm-btn doc-footer">
            <a href="{!! route('parking-list') !!}" class="btn btn-default"><i
...
@section('scripts')
    ...
    <script>
        ...
        var bookedSlot = [];
        var bookedSlotId = [];
        var refBooking = database.collection('booked_parking_order').where('parkingId', '==', id).where('status', 'in', ['placed', 'onGoing']).get().then(async function (snapshot) {
            if (snapshot.docs.length > 0) {
                snapshot.docs.forEach((listval) => {
                    ...
                    bookedSlot.push(val);
                    bookedSlotId.push(val.parkingSlotId);
        ...
        async function getSlotData(val = '') {
            ...
            var ridesRef = await database.collection('booked_parking_order').where('parkingId', '==', id).where('status', 'in', ['placed', 'onGoing']);
            var startQuery = ridesRef.where('bookingStartTime', '>=', startfirebaseTimestamp).where('bookingStartTime', '<=', endFirebaseTimestamp);

            $('.parking_slot').text('').addClass('parking-available ');
            $('.parking_slot').append('{{trans("lang.available")}}');
            var finalResults = [];
            ...
            if (requestId != '') {
                ...
                ref.get().then(async function (snapshots) {
                    if (snapshots.docs.length > 0) {
                        ...
                        $('.parking-ratings').html('<span class="badge badge-warning text-white dr-review"><i class="fa fa-star"></i>' + (rating).toFixed(1) + '</span>');

                        $('.location').text(data.address);

                        $('.description').text(data.description);
                        $('.space').text(data.parkingSpace);
                        $('.type').text(data.parkingType + " {{trans('lang.wheeler')}}");
                        ...
                        var space = data.parkingSpace;
                        var slotHtml = '';

                        for (var i = 1; i <= space; i++) {
                            var slotId = "A-" + i;
                            slotHtml += '<div class="col-lg-3 col-md-3 col-12"><div class="parking-slot-block"><div class="d-flex align-items-center parking-slot-box">';
                            slotHtml += '<a href="javascript:void(0)" id="redirect_' + slotId + '"><span class="slot-number">' + slotId + '</span>';

                            slotHtml += '<span class="slotcar-img parking-available parking_slot" id="A-' + i + '">{{trans("lang.available")}}</span>';
        ...
        async function getAssignUserDetails(parkingId) {
            var userName = '';
            await database.collection('users').where('parkingId', '==', parkingId).get().then(async function (snapshot) {
                if (snapshot.docs.length > 0) {
        ...
        function getUserName(userId) {
            database.collection('users').where('id', '==', userId).get().then(async function (snapshot) {
                ...
                if (snapshot.docs.length) {
                    var data = snapshot.docs[0].data();
                    $('.parking-owner').text(data.fullName);
                } else {
                    $('.parking-owner').text("{{trans('lang.unknown_user')}}");
                }
        ...
        async function getBookingInfo(slotId) {
            var bookingId = '';
            await database.collection('booked_parking_order').where('parkingId', '==', id).where('parkingSlotId', '==', slotId).where('status', '==', 'onGoing').get().then(async function (snapshot) {
                if (snapshot.docs.length > 0) {
        ...
        $("#assign-user-btn").click(async function () {
            ...
            if (id) {
                await database.collection('users').where('parkingId', '==', id).get().then(function (snapshots) {
            ...
            if (check_flag) {
                ...
                $(".error_top").show();
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.parking_assign_error')}}</p>");
                window.scrollTo(0, 0);
            }
