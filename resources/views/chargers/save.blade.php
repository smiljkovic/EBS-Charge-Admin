    ...
    <div class="page-wrapper">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h3 class="text-themecolor">{{trans('lang.charger_plural')}}</h3>
            </div>
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a
                            href="{!! route('chargers-list') !!}">{{trans('lang.charger_plural')}}</a>
                    </li>
                    <li class="breadcrumb-item active">{{ $id == ''? trans('lang.charger_create') :
                    trans('lang.charger_edit')}}
                    </li>
                </ol>
            </div>
        </div>
                ...
                <div class="card-body">
                    ...
                    <div class="row restaurant_payout_create">
                        <div class="restaurant_payout_create-inner">
                            <fieldset>
                                <legend>{{trans('lang.charger_details')}}</legend>
                                @if($id=='')
                                ...
                                <div class="form-group row width-50">
                                    <label class="col-3 control-label">{{trans('lang.charger_for')}}<span
                                            class="required-field"></span></label>
                                    <div class="col-7">
                                        <select class="form-control type" id="type">
                                            <option value="">{{trans("lang.select_charger_type")}}</option>
                                ...
                                <div class="form-group row width-50">
                                    <label class="col-3 control-label">{{trans('lang.charger_space')}}<span
                                            class="required-field"></span></label>
                                    <div class="col-7">
                                ...
                                <!-- Nikola - replaced charger_facilities with charging_facilities -->
                                <!-- <div class="form-group row width-100">
                                    <label class="col-3 control-label">{{trans('lang.charger_facilities')}}</label>
                                    <div class="facilities"></div>
                                </div> -->
                                <div class="form-group row width-100">
                                    <label class="col-3 control-label">{{trans('lang.charging_facilities')}}</label>
                                    <div class="facilities"></div>
                                </div>
                    ...
                    <div class="form-group col-12 text-center btm-btn">
                        <button type="button" class="btn btn-primary  create_charger_btn"><i class="fa fa-save"></i> {{
                        trans('lang.save')}}
                        </button>
                        <a href="{!! route('chargers-list') !!}" class="btn btn-default"><i class="fa fa-undo"></i>{{
                        trans('lang.cancel')}}</a>
                    </div>
    ...
    <script>
        ...
        ref.get().then(async function (snapshots) {
            ...
            snapshots.docs.forEach((listval) => {
                ...
                // TODO parking_facility class
                checkbox = checkbox + '<div class="form-check width-100"><input type="checkbox" name="charger_facility" class="charger_facility" id="' + data.id + '" data-name="' + data.name + '" data-image="' + data.image + '" data-enable="' + data.isEnable + '"><label class="col-3 control-label" for="' + data.id + '" >' + data.name + '</label></div>';
            })
        ...
        $(".create_charger_btn").click(function () {
            ...
            var bookmarkedUser = [];
            var userId = $('.user').val();
            var checkboxes = document.getElementsByClassName('charger_facility');
            ...
            if (name == '') {
                $(".error_top").show();
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.charger_name_error')}}</p>");
                window.scrollTo(0, 0);
                ...
                $(".error_top").show();
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.charger_address_error')}}</p>");
                window.scrollTo(0, 0);
                ...
                $(".error_top").show();
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.charger_type_error')}}</p>");
                window.scrollTo(0, 0);

            } else if (chargerPrice == '') {
                $(".error_top").show();
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.charger_price_error')}}</p>");
                window.scrollTo(0, 0);
                ...
                requestId == ''
                    ? storeImageData().then(IMG => {
                        geoFirestore.collection('chargers').doc(id).set({
                            'id': id,
                            'isEnable': enable,
                            'name': name,
                            'image': IMG,
                    ...
                    : storeImageData().then(IMG => {
                        geoFirestore.collection('chargers').doc(id).update({
                            'isEnable': enable,
                            'name': name,
                            'image': IMG,
                            coordinates: new firebase.firestore.GeoPoint(lat, long),
