    ...
    <div class="page-wrapper">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h3 class="text-themecolor orderTitle">{{trans('lang.chargers_list')}}</h3>
            </div>
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{url('/dashboard')}}">{{trans('lang.dashboard')}}</a></li>
                    <li class="breadcrumb-item active">{{trans('lang.chargers_list')}}</li>
                </ol>
            </div>
            <div>
            </div>
                            ...
                            <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
                                <li class="nav-item">
                                    <a class="nav-link active" href="#"><i
                                            class="fa fa-list mr-2"></i>{{trans('lang.chargers_list')}}</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" href="{{route('chargers-list.save')}}"><i
                                            class="fa fa-plus mr-2"></i>{{trans('lang.charger_create')}}</a>
                                </li>
                            </ul>
                        </div>
                        <div class="card-body">
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        var refData = database.collection('chargers');
        ...
        async function getListData(val) {
            ...
            total = await getBookingCount(val.id); // TODO - handle bookings
            ...
            html += '<td class="action-btn"><a href="' + charger_view + '"><i class="fa fa-eye"></i></a><a href="' + route1 + '"><i class="fa fa-edit"></i></a><a id="' + val.id + '" class="do_not_delete" href="javascript:void(0)" name="parking-delete"><i class="fa fa-trash"></i></a></td>';
            html += '</tr>';
            return html;
        }
        ...
        async function getBookingCount(id) {
            var count = 0;
            await database.collection('booked_parking_order').where('parkingId', '==', id).get().then(async function (snapshot) { // TODO Nikola - hangle bookings
        ...
        $("#is_active").click(function () {
            $("#chargerTable .is_open").prop('checked', $(this).prop('checked'));

        });

        $("#deleteAll").click(function () {
            if ($('#chargerTable .is_open:checked').length) {
                if (confirm("{{trans('lang.selected_delete_alert')}}")) {
                    jQuery("#overlay").show();
                    $('#chargerTable .is_open:checked').each(function () {
                        var dataId = $(this).attr('dataId');
                        database.collection('chargers').doc(dataId).delete().then(function () {
                            setTimeout(function () {
        ...
        $(document).on("click", "a[name='charger-delete']", function (e) {

            var id = this.id;
            jQuery("#overlay").show();
            database.collection('chargers').doc(id).delete().then(function (result) {
        ...
        $(document).on("click", "input[name='isEnabled']", function (e) {
            ...
            var id = this.id;
            database.collection('chargers').doc(id).update({'isEnable': ischeck ? true : false}).then(function (result) {
            });
        });
