    ...
    <script>
        ...
        customerRef.get().then(function (snapShots) {
            ...
            if (snapShots.docs.length > 0) {
                ...
                $('.user').append('<option value="all" selected>{{trans("lang.all_user")}}</option>');
            ...
            if (fileFormat == '') {
                ...
                userRef.get().then(async function (userSnapshots) {
                    ...
                    if (userSnapshots.docs.length > 0) {
                        var transactionData = await getData(userSnapshots);
        ...
        async function getData(querySnapshot) {
            ...
            await Promise.all(querySnapshot.docs.map(async (doc) => {
                ...
                var newUserObj = {};
                var userName = await getUserData(userObj.userId);
