        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">{{trans('lang.model_table')}}</li>
                </ol>
            </div>
            <div>
            </div>
                            ...
                            <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
                                <li class="nav-item">
                                    <a class="nav-link active" href="{!! url()->current() !!}"><i
                                            class="fa fa-list mr-2"></i>{{trans('lang.model_table')}}</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" href="{!! url('/model/save') !!}"><i
                                            class="fa fa-plus mr-2"></i>{{trans('lang.model_create')}}</a>
                                </li>
                            </ul>
                        </div>
                        <div class="card-body">
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        async function getListData(val) {
            ...
            newdate = '';
            var id = val.id;
            var route1 = '{{route("model.save",":id")}}';
            route1 = route1.replace(':id', id);

            html = html + '<td class="delete-all"><input type="checkbox" id="is_open_' + id + '" class="is_open" dataId="' + id + '"><label class="col-3 control-label"\n' +
                'for="is_open_' + id + '" ></label></td>';
            var src = "{{asset('images/default_user.png')}}";
            if (val.hasOwnProperty('image')) {
                html = html + '<td><img style="width:50px" class="rounded" src="' + val.image + '"></td>';
            ...
            brand = await getBrand(val.brandId);
            ...
            html = html + '<td class="action-btn"><a href="' + route1 + '"><i class="fa fa-edit"></i></a><a id="' + val.id + '" class="do_not_delete" name="model-delete" href="javascript:void(0)"><i class="fa fa-trash"></i></a></td>';
            html = html + '</tr>';
            return html;
        }
        ...
        $(document).on("click", "input[name='isEnabled']", function (e) {
            ...
            var id = this.id;
            database.collection('vehicle_model').doc(id).update({'enable': ischeck ? true : false}).then(function (result) {
            });
        });

        $(document).on("click", "a[name='model-delete']", function (e) {
