    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            paymentRef.get().then(async function (paymentSnapshots) {
                ...
                $('.razorpay_key').val(payment.razorpayKey);
                $('.razorpay_secret').val(payment.razorpaySecret);
        ...
        $(".save_razorpay_btn").click(function () {
            ...
            var publicKey = $(".razorpay_key").val();
            var secretKey = $(".razorpay_secret").val();
            var isEnabled = $("#enable_razorpay").is(":checked");
            var isSandBox = $("#sand_box_mode").is(":checked");
            storeImageData().then(IMG => {
                database.collection('settings').doc("payment").update({
                    'razorpay.enable': isEnabled,
                    'razorpay.isSandbox': isSandBox,
                    'razorpay.razorpayKey': publicKey,
                    'razorpay.razorpaySecret': secretKey,
                    'razorpay.image': IMG
                }).then(function (result) {
                    window.location.href = '{{ url("settings/payments/razorpay")}}';
                });
            }).catch(err => {
                jQuery("#overlay").hide();
                $(".error_top").show();
