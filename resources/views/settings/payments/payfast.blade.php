                ...
                <ul class="nav nav-tabs card-header-tabs align-items-end">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active payfast_active_label"
                           href="{!! url('settings/payments/payfast') !!}"><i
                ...
                <div class="row restaurant_payout_create">
                    <div class="restaurant_payout_create-inner">
                        <fieldset>
                            ...
                            <div class="form-group row width-100">
                                <label
                                    class="col-3 control-label">{{trans('lang.app_setting_payfast_cancel_url')}}</label>
                                <div class="col-7">
                            ...
                            <div class="form-group row width-100">
                                <label
                                    class="col-3 control-label">{{trans('lang.app_setting_payfast_notify_url')}}</label>
                                <div class="col-7">
                            ...
                            <div class="form-group row width-100">
                                <label
                                    class="col-3 control-label">{{trans('lang.app_setting_payfast_return_url')}}</label>
                                <div class="col-7">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            paymentRef.get().then(async function (paymentSnapshots) {
                var payment = paymentSnapshots.data().payfast;
                if (payment.enable) {
                ...
                $('.merchant_key').val(payment.merchantKey);
                $('.merchant_id').val(payment.merchantId);
                $('.cancel_url').val(payment.cancel_url);
                $('.notify_url').val(payment.notify_url);
                $('.return_url').val(payment.return_url);
        ...
        $(".save_payfast_btn").click(function () {
            ...
            var merchantKey = $(".merchant_key").val();
            var merchantId = $(".merchant_id").val();
            var cancel_url = $(".cancel_url").val();
            var notify_url = $(".notify_url").val();
            var return_url = $(".return_url").val();
            var isEnabled = $("#enable_payfast").is(":checked");
            var isSandBox = $("#sand_box_mode").is(":checked");
            storeImageData().then(IMG => {
                database.collection('settings').doc("payment").update({
                    'payfast.enable': isEnabled,
                    'payfast.isSandbox': isSandBox,
                    'payfast.merchantKey': merchantKey,
                    'payfast.merchantId': merchantId,
                    'payfast.cancel_url': cancel_url,
                    'payfast.notify_url': notify_url,
                    'payfast.return_url': return_url,
                    'payfast.image': IMG
                }).then(function (result) {
                    window.location.href = '{{ url("settings/payments/payfast")}}';
                });
            }).catch(err => {
                jQuery("#overlay").hide();
                $(".error_top").show();
