                ...
                <ul class="nav nav-tabs card-header-tabs align-items-end">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active paystack_active_label"
                           href="{!! url('settings/payments/paystack') !!}"><i
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            paymentRef.get().then(async function (paymentSnapshots) {
                var payment = paymentSnapshots.data().payStack;
                if (payment.enable) {
                ...
                $('.callbackURL').val(payment.callbackURL);
                $('.webhookURL').val(payment.webhookURL);
        ...
        $(".save_paystack_btn").click(function () {
            ...
            var isEnabled = $("#enable_paystack").is(":checked");
            var isSandBox = $("#sand_box_mode").is(":checked");
            storeImageData().then(IMG => {

                database.collection('settings').doc("payment").update({
                    'payStack.enable': isEnabled,
                    'payStack.isSandbox': isSandBox,
                    'payStack.secretKey': secretKey,
                    'payStack.publicKey': publicKey,
                    'payStack.callbackURL': callbackURL,
                    'payStack.webhookURL': webhookURL,
                }).then(function (result) {
                    window.location.href = '{{ url("settings/payments/paystack")}}';
                });
            }).catch(err => {
                jQuery("#overlay").hide();
                $(".error_top").show();
