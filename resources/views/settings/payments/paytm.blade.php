        ...
        <div class="card">
            <div class="payment-top-tab mt-3 mb-3">
                <ul class="nav nav-tabs card-header-tabs align-items-end">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active paytm_active_label" href="{!! url('settings/payments/paytm') !!}"><i
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            paymentRef.get().then(async function (paymentSnapshots) {
                var payment = paymentSnapshots.data().paytm;
                if (payment.enable) {
                ...
                $('.paytm_merchant_key').val(payment.merchantKey);
                $('.paytm_id').val(payment.paytmMID);
        ...
        $(".save_paytm_btn").click(function () {
            ...
            var merchantKey = $(".paytm_merchant_key").val();
            var paytmMID = $(".paytm_id").val();
            var isEnabled = $("#enable_paytm").is(":checked");
            var isSandBox = $("#enable_paytm_sendbox").is(":checked");
            storeImageData().then(IMG => {
                database.collection('settings').doc("payment").update({
                    'paytm.enable': isEnabled,
                    'paytm.isSandbox': isSandBox,
                    'paytm.merchantKey': merchantKey,
                    'paytm.paytmMID': paytmMID,
                    'paytm.image': IMG
                }).then(function (result) {
                    window.location.href = '{{ url("settings/payments/paytm")}}';
                });
            }).catch(err => {
                jQuery("#overlay").hide();
                $(".error_top").show();
